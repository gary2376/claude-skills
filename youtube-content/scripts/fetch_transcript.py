...
Usage:
    python3 fetch_transcript.py <url_or_video_id> [--language en,tr] [--timestamps] [--text-only]
...
Output (JSON, default mode):
    {
        ...
        # "timestamped_text" is also included when --timestamps is passed
    }

With --text-only, prints plain text (or timestamped text if --timestamps is also
passed) instead of JSON.

Install dependency:  pip install youtube-transcript-api  (or: uv pip install youtube-transcript-api)
"""
...
def fetch_transcript(video_id: str, languages: list = None):
    ...
    try:
        ...
        print(json.dumps({"error": "youtube-transcript-api not installed. Run: pip install youtube-transcript-api"}))
        sys.exit(1)
